        ...
        <div className="space-y-8 font-sans">
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-rose-50 rounded-2xl">
                    <p className="text-gray-500">No events scheduled. Check back soon!</p>
                </div>
            ) : (
                    ...
                    {events.map((event) => (
                        <div key={event.id} className="bg-white rounded-2xl p-6 border border-rose-100 shadow-sm hover:shadow-lg transition-all flex flex-col md:flex-row gap-6 items-start">
                            ...
                            <div className="w-full md:w-48 h-48 rounded-xl overflow-hidden shrink-0 relative">
                                <img src={event.image || 'https://images.unsplash.com/photo-1510972527921-ce03766a1cf1?w=800&auto=format&fit=crop&q=60'} alt={event.title} className="w-full h-full object-cover" />
